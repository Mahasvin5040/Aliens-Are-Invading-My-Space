...
document.addEventListener("keyup", (e) => {
    if (e.key == "ArrowLeft" || player.pos.x <= 0) {
        movingLeft = false;
    }

    if (e.key == "ArrowRight" || player.pos.x >= canvas.width - player.pos.x) {
        movingRight = false;
    }
});
...
function createPlayerLaser(player){
    const object = new createObject(0, friendlyLaser, 8, 20, player.pos.x+player.size.x/2-4, player.pos.y, 0, -70);
    objects.push(object);
}
...
function playerShot(){
    ...
    if(player.shootingTimer > 60 * player.playerShootingTime){
